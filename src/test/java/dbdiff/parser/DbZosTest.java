...
class DbZosTest {
    ...
    static Stream<Arguments> tableDescLinesProvider() {
        return Stream.of(
                ...
                arguments("-- Справочник документов, подтверждающих право на строительство:AXAD", "Справочник документов, подтверждающих право на строительство", "AXAD"),
                arguments("-- Приложение 21:AN21", "Приложение 21", "AN21")
        );
    }
    ...
    static Stream<Arguments> columnDescLinesProvider() {
        return Stream.of(
                ...
                arguments("-- ID", "ID"),
                arguments("-- Причина порчи/утраты сертификата", "Причина порчи/утраты сертификата"),
                arguments("-- Очередность рождения серт. ребенка", "Очередность рождения серт. ребенка"),
                arguments("-- Причина изменений МКАД (вместо CACR_ID)", "Причина изменений МКАД (вместо CACR_ID)")
        );
    }
